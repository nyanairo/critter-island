// Training: 5 menus. Each menu produces a deterministic-ish result given
// the monster's growth + fatigue. Caller advances week via state.tickWeekForTraining.
...
export function computeTraining(monster, menuId, weekSeed) {
  ...
  // Fatigue penalty: above 60 fatigue gains halve.
  ...
  // Side gain: randomly bump one other stat by a small amount.
  ...
  message = tired
    ? `${monster.name} は疲れぎみ… ${menu.label} +${focusGain}`
